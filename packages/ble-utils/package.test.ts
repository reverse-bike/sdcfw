...
const FIRMWARE_DIR = new URL(
  "../../firmware/mc/230-BLUETOOTH-EXT1-310/",
  import.meta.url,
);
...
test("official controller package parses and verifies its reversed SHA-256", async () => {
  const dat = new Uint8Array(
    await Bun.file(
      new URL("GD_S73Rx_H104_S310US_20221020.dat", FIRMWARE_DIR),
    ).arrayBuffer(),
  );
  const bin = new Uint8Array(
    await Bun.file(
      new URL("GD_S73Rx_H104_S310US_20221020.bin", FIRMWARE_DIR),
    ).arrayBuffer(),
  );
