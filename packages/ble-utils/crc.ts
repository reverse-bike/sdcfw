...
export function crc32Mpeg2Update(data: Uint8Array, initialCrc = 0xffffffff): number {
  let crc = initialCrc >>> 0;
  for (const byte of data) {
    crc ^= byte << 24;
    for (let bit = 0; bit < 8; bit++) {
      crc = crc & 0x80000000 ? ((crc << 1) ^ MPEG2_POLY) >>> 0 : (crc << 1) >>> 0;
    }
  }
  return crc >>> 0;
}
...
const IEEE_TABLE = (() => {
  ...
  for (let index = 0; index < table.length; index++) {
    let crc = index;
    for (let bit = 0; bit < 8; bit++) {
      crc = crc & 1 ? (0xedb88320 ^ (crc >>> 1)) >>> 0 : (crc >>> 1) >>> 0;
    }
    table[index] = crc;
  }
  return table;
