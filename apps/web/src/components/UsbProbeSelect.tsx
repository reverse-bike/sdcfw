...
export default function UsbProbeSelect(props: UsbProbeSelectProps) {
  ...
  const getDeviceName = (device: USBDevice): string => {
    return (
      device.productName ||
      `Unknown Device (${device.vendorId?.toString(16)}:${device.productId?.toString(16)})`
    );
  };
  ...
  const refreshDevices = async () => {
    ...
    try {
      ...
      const filteredDevices = authorizedDevices.filter(isDapLinkDevice).map((device) => ({
        device,
  ...
  const requestDevice = async () => {
    ...
    try {
      ...
      setError(
        `Failed to connect to device: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
