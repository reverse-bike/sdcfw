...
export default function Backup(props: BackupProps) {
  ...
  const downloadBackup = async () => {
    ...
    try {
      ...
      setError(
        `Failed to create download: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
  ...
  const formatDeviceInfo = (info: DeviceInfo) => {
    ...
    const variantStr = new TextDecoder().decode(variantBuf).replaceAll(String.fromCharCode(0), "");
  ...
  const content = (
    <>
      ...
      <Show when={progress()}>
        <div class="mb-4">
          <ProgressBar message={progress()} percent={progressPercent()} variant="green" />
        </div>
      </Show>
