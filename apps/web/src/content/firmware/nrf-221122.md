...
This is the first custom firmware for the Diamond Display. It's based on NRF version 221122, which is the "torque update". It's patched to default startup in mode 4 (off road mode), instead of mode 2 (US Only, for now). Additionally, this firmware has been patched with a new public key (SuperDuper Skeleton Key) to allow for future patch updated to be applied via Bluetooth.
